...
## [2026-06-26] поправка к аудиту: один промптящий путь всё же был — `run_tests mode=PlayMode`

Аудит T1b проверял scene-**тулы** (наши прямые вызовы API), но пропустил, что `run_tests mode=PlayMode`
делегирует Unity **Test Runner**'у, а тот перед PlayMode-прогоном сам зовёт «Save scene?»-модал на грязной
сцене. Это и был реальный триггер зависаний (подтверждено `ping`: responsive=False, mode=edit). Закрыто
guard'ом в `RunTestsTool` (грязная-с-путём → молча save; untitled → ранний отказ-ошибка вместо модала) —
детали в [[m3-playmode-reload]]/journal. Вывод аудита «наши тулы модал не открывают» теперь верен полностью:
последний путь, который мог, обезврежен. AC4.9 закрыт по факту для всех тулов, включая PlayMode-прогон.
