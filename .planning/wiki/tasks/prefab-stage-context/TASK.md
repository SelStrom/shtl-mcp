...
Editor-скрипт на 934 строки в host-проекте, написанный **только** ради того,
что через MCP нельзя развести внутрипрефабные ссылки. Симптом со стороны модели: `get_selection`
возвращает `Canvas (Environment)/SelectionPanel`, а `get_object` на тот же путь отвечает
