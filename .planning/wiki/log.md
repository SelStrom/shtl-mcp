...
**Сторонние проекты в журнале не упоминаются — ни реальные, ни вымышленные**, в том
числе как примеры. Репозиторий публичный. Повод задачи описывается обезличенно
(«host-проект», «проект-потребитель»): без названий, ключей трекера, имён файлов и
цитат кода чужих проектов. То же правило — для `tasks/*/journal.md` и `TASK.md`
(см. `tasks/README.md`).
...
## [2026-08-03] forward | serverName закреплён за путём проекта | sticky-server-name
F1/AC1.3 переформулирован + AC1.8: имя инстанса назначается один раз и переиспользуется, пока жива
запись в реестре. Было — вычисление на каждом старте из «кто сейчас жив», то есть базовое
`unity-<productName>` доставалось тому, кто раньше открыл Unity; у клонов productName одинаков.
Асимметрия с портом (детерминирован по пути, AC1.4) и ломала адрес: клиентская запись `claude mcp add`
ключуется именем, поэтому после смены владельца имени она начинала означать другой проект, а повторное
добавление из дашборда плодило второе имя на тот же порт. Носитель закрепления — сам `registry.json`:
записи durable (`Prune` только в тестах, `Remove` без вызовов в проде), поэтому миграция бесплатная и
не переименовывает ни один существующий инстанс. Попутно средняя ступень минта: при занятом базовом
имени новый путь получает `unity-<имя папки>` вместо хеша пути, hash4 остался
фолбэком. Логика прогнана вне Unity 14/14 (вкл. пути реальной multi-checkout установки); EditMode не гонялся —
тестовая сборка под `SHTL_MCP_DEV`, `TestProject~` не поднят.


## [2026-08-05] forward | set_selection принимает asset-path | set-selection-assets
F3/AC3.1: резолв target'а в `set_selection` стал двухступенчатым — объект текущего контекста, иначе
`AssetDatabase.LoadAssetAtPath`. Было единственное исключение из общей формы: `get_object`/`modify_object`
принимали объект контекста, asset-path и instanceId (AC3.11), а выделение — только объект сцены, поэтому
ассет нельзя было ни показать человеку в Project-окне, ни подать на вход `MenuItem`, работающему по
`Selection.objects`; оставался `run_csharp` — тул за footgun-флагом. Приоритет у объекта контекста, так что
прежние вызовы не меняют поведения; ненайденная строка по-прежнему уходит в `missing`. `instanceId` не
добавлен: массив строк не хочется расширять до разнотипных элементов ради формы, которой не просили.
