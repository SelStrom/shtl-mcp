...
namespace Shtl.Mcp.UI
{
    public sealed class DashboardWindow : EditorWindow
    {
        ...
        const float SectionGap = 4;  // единый вертикальный отступ между блоками дашборда
        ...
        {
            var s = ShtlMcpServer.Instance;
            return Shtl.Mcp.Server.ClaudeCli.AddUserScopeArgs(s.ServerName, s.Port);
        }
        ...
        // CLI-раннер вынесен в Shtl.Mcp.Server.ClaudeCli (переиспользуется lifecycle для ре-регистрации порта).
        static (int exit, string output) RunClaude(string args, int timeoutMs)
            => Shtl.Mcp.Server.ClaudeCli.Run(args, timeoutMs);
